        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6 overflow-y-auto">
          <div className="space-y-4">
            ...
            <div className="space-y-4 pt-4">
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Gestión de Documentos</label>
              
              {/* Documentos Existentes */}
              {existingDocs.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase">Archivos actuales</p>
                  <ul className="space-y-2">
                    {existingDocs.map((doc, i) => (
                      <li key={i} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-lg text-sm">
                        ...
                        <button 
                          ...
                          className="p-1 text-slate-400 dark:text-slate-500 cursor-pointer hover:text-red-500 dark:hover:text-red-400 transition-colors"
              ...
              {newFiles.length > 0 && (
                <ul className="mt-2 space-y-2">
                  {newFiles.map((file, i) => (
                    <li key={i} className="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/50 rounded-lg text-sm">
                      <div className="flex items-center gap-2 text-blue-700 dark:text-blue-400">
                        <FileIcon className="w-4 h-4" />
                        <span className="truncate">{file.name}</span>
                        <span className="text-[10px] bg-blue-200 dark:bg-blue-800 px-1.5 rounded uppercase font-bold text-blue-800 dark:text-blue-100">Nuevo</span>
                      </div>
                      <button 
                        type="button"
                        onClick={() => removeNewFile(i)}
                        className="p-1 text-blue-400 dark:text-blue-500 cursor-pointer hover:text-red-500 dark:hover:text-red-400 transition-colors"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </li>
